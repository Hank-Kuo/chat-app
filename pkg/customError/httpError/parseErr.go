...
func ParseError(err error) *Err {
	...
	case errors.Is(err, customError.ErrInvalidJWTClaims):
		return &Err{
			Status: http.StatusUnauthorized, Message: customError.ErrInvalidJWTClaims.Error(),
		}
	case errors.Is(err, customError.ErrNotGetJWTToken):

		return &Err{
			Status: http.StatusUnauthorized, Message: customError.ErrNotGetJWTToken.Error(),
		}
